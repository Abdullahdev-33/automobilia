...
class ChatController extends Controller{
    ...
    public function chat_messages(String $id){
        ...
        $messages = Message::where('chat_id',$id)->orderBy('id' , 'desc')->paginate(50);
            ...
            return $message->created_at->format('D M d, Y'); 
        });
    ...
    public function create_Chat(Request $request, string $id){
       ...
       $chat = Chat::create([
        ...
        if(!$chat){
            return abort(500, "Chat Creation Failed");
        }

        return redirect()->route('chat.index');
    }
    public function delete_Chat(string $id){
        $chat = Chat::find($id);
        if(!$chat){
