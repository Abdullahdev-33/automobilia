        ...
        document.addEventListener('DOMContentLoaded', ()=>{
            ...
            async function fetchMessageForChat(id , chat_id){
              ...
              if(chat_id != id){
                ...
                Object.entries(array.messages.data).reverse().forEach(([date, messages]) => {
    
                    // Date header
                    const dateDiv = document.createElement('div');
                    dateDiv.className = 'my-4 bg-slate-200 rounded shadow-md w-fit mx-auto py-1 px-3 text-center text-sm text-gray-500';
                    dateDiv.textContent = date;
                    message_container.appendChild(dateDiv);

                    messages.reverse().forEach((message) => {
                        // Insert "unread" marker if applicable
                        if ((message.id > array.last_seen_id.message_id) && (message.sender_id != user_id) && unread_marker) {
                            ...
                            unreadDiv.className = 'w-full my-3 text-center border-b border-t py-1 border-t-slate-300 border-b-slate-300';
                            unreadDiv.textContent = 'Unread messages';
                            message_container.appendChild(unreadDiv);
                            unread_marker = false;
                        }
                ...
                next_button.addEventListener('click' , ()=>{
                    if(!nextPageUrl) return;
                        ...
                        .then((response)=>{
                             ...
                             let previousScrollHeight = message_container.scrollHeight

                             let groupedMessages = response.messages.data;
                             let html = ""
                             Object.entries(groupedMessages).reverse().forEach(([date, messages])=>{
                                html += `<div class="my-4 bg-slate-200 rounded shadow-md w-fit mx-auto py-1 px-3 text-center text-sm text-gray-500 >${date}</div>`
                                html += messages.reverse().map(msg => {
                            ...
                            message_container.scrollTop = message_container.scrollHeight - previousScrollHeight;
